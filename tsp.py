...
if _vrp_active_report and _report_vehicle_stats:
    with st.sidebar:
        st.divider()
        st.subheader("Resultado por veiculo")
        for vs in _report_vehicle_stats:
            _weight_ok = _vehicle_capacity_weight is None or vs.weight <= _vehicle_capacity_weight
            _volume_ok = _vehicle_capacity_volume is None or vs.volume <= _vehicle_capacity_volume
            _status_icon = "OK" if (_weight_ok and _volume_ok) else "VIOLACAO"
            with st.expander(f"Veiculo {vs.vehicle_id} ({len(vs.cities)} cidades) [{_status_icon}]"):
                st.write(f"Distancia: {vs.distance:.2f}")
                if _vehicle_capacity_weight is not None:
                    _pct_w = (vs.weight / _vehicle_capacity_weight * 100) if _vehicle_capacity_weight > 0 else 0
                    st.write(f"Peso: {vs.weight:.2f} / {_vehicle_capacity_weight:.1f} kg ({_pct_w:.0f}%)")
                    if not _weight_ok:
                        st.error(f"Peso excede capacidade em {vs.weight - _vehicle_capacity_weight:.2f} kg")
                else:
                    st.write(f"Peso: {vs.weight:.2f} kg (sem limite)")
                if _vehicle_capacity_volume is not None:
                    _pct_v = (vs.volume / _vehicle_capacity_volume * 100) if _vehicle_capacity_volume > 0 else 0
                    st.write(f"Volume: {vs.volume:.4f} / {_vehicle_capacity_volume:.2f} m3 ({_pct_v:.0f}%)")
                    if not _volume_ok:
                        st.error(f"Volume excede capacidade em {vs.volume - _vehicle_capacity_volume:.4f} m3")
                else:
                    st.write(f"Volume: {vs.volume:.4f} m3 (sem limite)")
                st.write(f"Retornos ao deposito: {vs.depot_returns}")
                if vs.cities:
                    _city_indices = [cities_locations.index(c) for c in vs.cities if c in cities_locations]
                    st.caption(f"Cidades: {_city_indices}")
                    _priorities_in_route = [
                        _city_priorities[ci] for ci in _city_indices if ci < len(_city_priorities)
                    ]
                    _crit_count = sum(1 for p in _priorities_in_route if p == "critical_meds")
                    if _crit_count > 0:
                        _first_crit = next(
                            (i for i, p in enumerate(_priorities_in_route) if p == "critical_meds"), None
                        )
                        _first_reg = next(
                            (i for i, p in enumerate(_priorities_in_route) if p != "critical_meds"), None
                        )
                        if _first_crit is not None and _first_reg is not None and _first_reg < _first_crit:
                            st.warning("Prioridade: cidade critica posicionada apos cidade regular nesta rota")
                        else:
                            st.success(f"Prioridade: {_crit_count} entrega(s) critica(s) atendida(s) primeiro")
